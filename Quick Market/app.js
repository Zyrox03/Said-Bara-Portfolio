...
for(let i=0; i< lilImages.children.length;i++){
    if(lilImages.children[i].classList.contains('current') == true){
        mainImage.children[0].src =  lilImages.children[i].children[0].src
...
function goTargetImage(e){

    let targetImg = e.target;

    if(targetImg.classList.contains('lilImages') == true){
        return ;
    }else{
    ...
    current.classList.remove('current') ;
    targetImg.parentElement.classList.add('current')
     current = document.querySelector('.current');

     setTimeout(function(){
        mainImage.children[0].src =  targetImg.src
...
let productPrice = document.querySelector('.product_description b span').innerText
let total = document.querySelector('.total span');

let livraison = document.querySelector('.livraison span');

total.innerText = parseFloat(productPrice);



form.addEventListener('input', function () {

    if (Wilaya.value === "Béjaïa") {

        livraison.innerText = '300'
    } else if (Wilaya.value === "Alger") {

        livraison.innerText = '400'
    } else {
        livraison.innerText = '600'

    }

    total.innerText = (parseFloat(productPrice) * document.querySelector('.qty').children[1].value) + parseFloat(livraison.innerText);
...
let buyNowBtn = document.querySelector('.buyNow').children[0].children[0];
...
setTimeout(function(){
    buyNowBtn.style.transform = 'translateX(5px)';
...
setTimeout(function(){
    buyNowBtn.style.transform = 'translateX(0px)'
},2000)
